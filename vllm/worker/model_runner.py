...
class GPUModelRunnerBase(ModelRunnerBase[TModelInputForGPU]):
    ...
    def get_powv(self, verifyChatCompletion):
        powv = 0
        if verifyChatCompletion:
            logging.debug("Starting PoWV calculation with tensor parallelism...")
            
            # Aggregating from all tensor parallel shards
            weight_indices = self._gather_sharded_weights()
            token_sum = self._calculate_token_sum_across_gpus()
            
            powv = floor(weights[weight_index] * token_sum)
            
            logging.debug(f"PoWV calculated: {powv} from weight index {weight_index} and token sum {token_sum}")
        return powv

    def _gather_sharded_weights(self):
        logging.debug("Gathering sharded weights across tensor parallel GPUs.")
        sharded_weights = self.model.get_sharded_weights()
        return torch.cat(sharded_weights, dim=0)

    def _calculate_token_sum_across_gpus(self):
        logging.debug("Calculating token sums across all tensor-parallel GPUs.")
        token_sums = self.model.get_sharded_token_sums()
        return sum(token_sums)
                    self,
